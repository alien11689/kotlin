...
fun isInsideInlineArgument(inlineArgument: JetFunction, location: Location, debugProcess: DebugProcessImpl): Boolean {
    return isInsideInlineArgument(inlineArgument, location.visibleVariables(debugProcess))
}

fun isInsideInlineArgument(inlineArgument: JetFunction, visibleVariables: List<LocalVariable>): Boolean {
...
private fun lambdaOrdinalIndex(elementAt: PsiElement): HashMap<JetFunction, Int> {
    val parent = elementAt.parents.firstIsInstanceOrNull<JetNamedFunction>()

    val actualLambdaOrdinals = hashMapOf<JetFunction, Int>()
    parent?.accept(object : JetTreeVisitorVoid() {
