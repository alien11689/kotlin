...
class KotlinCodeFragmentFactory: CodeFragmentFactory() {
    ...
    companion object {
        ...
        private fun getInlineLambdaIfInside(contextElement: JetElement): JetFunction? {
            val parentLambda = contextElement.getParentOfType<JetFunction>(false) ?: return null

            if (!parentLambda.isLocal) return null
            if (JetPsiUtil.getParentCallIfPresent(parentLambda) == null) return null
