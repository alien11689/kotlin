...
public open class JetPsiChecker : Annotator, HighlightRangeExtension {
    ...
    fun annotateElement(element: PsiElement, holder: AnnotationHolder, diagnostics: Diagnostics) {
        if (ProjectRootsUtil.isInProjectSource(element) || element.getContainingFile() is JetCodeFragment) {
            val elementAnnotator = ElementAnnotator(element, holder, { parameter: JetParameter -> shouldSuppressUnusedParameter(parameter) })
            elementAnnotator.registerDiagnosticsAnnotations(diagnostics.forElement(element))
        }
    }
...
fun createQuickFixes(similarDiagnostics: Collection<Diagnostic>): MultiMap<Diagnostic, IntentionAction> {
...
private class ElementAnnotator(private val element: PsiElement,
                               ...
                               private val shouldSuppressUnusedParameter: (JetParameter) -> Boolean) {
