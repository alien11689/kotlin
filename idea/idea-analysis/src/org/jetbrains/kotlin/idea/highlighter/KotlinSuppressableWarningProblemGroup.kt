...
private object DeclarationKindDetector : JetVisitor<AnnotationHostKind?, Unit?>() {
    ...
    override fun visitDeclaration(d: JetDeclaration, arg: Unit?) = null

    override fun visitClass(d: JetClass, arg: Unit?) = detect(d, if (d.isInterface()) "interface" else "class")

    override fun visitNamedFunction(d: JetNamedFunction, arg: Unit?) = detect(d, "fun")

    override fun visitProperty(d: JetProperty, arg: Unit?) = detect(d, d.getValOrVarKeyword().getText()!!)

    override fun visitMultiDeclaration(d: JetMultiDeclaration, arg: Unit?) = detect(d, d.getValOrVarKeyword()?.getText() ?: "val",
                                                                                  name = d.getEntries().map { it.getName()!! }.join(", ", "(", ")"))

    override fun visitTypeParameter(d: JetTypeParameter, arg: Unit?) = detect(d, "type parameter", newLineNeeded = false)

    override fun visitEnumEntry(d: JetEnumEntry, arg: Unit?) = detect(d, "enum entry")

    override fun visitParameter(d: JetParameter, arg: Unit?) = detect(d, "parameter", newLineNeeded = false)

    override fun visitObjectDeclaration(d: JetObjectDeclaration, arg: Unit?): AnnotationHostKind? {
