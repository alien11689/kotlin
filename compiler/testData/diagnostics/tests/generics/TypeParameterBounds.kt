...
interface CorrectF<T> where T : D<A>, T : <!BOUND_APPEARS_TWICE!>D<B><!>

interface G<T>

interface IncorrectH<<!MISPLACED_TYPE_PARAMETER_CONSTRAINTS!>T : G<D<A>><!>> where T : G<D<T>>

interface CorrectH<T> where T : G<D<A>>, T : <!BOUND_APPEARS_TWICE!>G<D<B>><!>
...
interface correctJ<T> where T : G<D<T>>, T : <!BOUND_APPEARS_TWICE!>G<D<T?>><!>