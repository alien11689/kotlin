 ...
 */
public fun CharSequence.subSequence(range: Range<Int>): CharSequence = subSequence(range.start, range.end + 1)

public fun CharSequence.substring(start: Int, end: Int = length()): String = subSequence(start, end).toString()

public fun CharSequence.substring(range: Range<Int>): String = subSequence(range.start, range.end + 1).toString()
...
public fun CharSequence.endsWith(suffix: CharSequence, ignoreCase: Boolean = false): Boolean {
    if (!ignoreCase && this is String && suffix is String)
        ...
        return regionMatchesImpl(length() - suffix.length(), suffix, 0, suffix.length(), ignoreCase = true)
}
